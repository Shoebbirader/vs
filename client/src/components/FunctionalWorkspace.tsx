...
export default function FunctionalWorkspace({ section, session, onBack, organizationName }: Props) {
  ...
  const content = section === "Team" ? <TeamWorkspace enabled={session} /> : session && section === "Settings" ? <OrganizationSettingsWorkspace /> : session && section === "Command center" ? <ExecutiveOverviewWorkspace organizationName={organizationName} /> : session && section === "Fleet manager workspace" ? <FleetManagerOverviewWorkspace organizationName={organizationName} /> : session && section === "Vehicles" ? <VehicleRegisterWorkspace /> : session && section === "Components" ? <ComponentLifecycleWorkspace /> : session && section === "Work orders" ? <WorkOrderReviewWorkspace organizationName={organizationName} /> : session && (section === "Inventory manager workspace" || section === "Inventory") ? <InventoryControlWorkspace /> : session && (section === "Mechanic workspace" || section === "Mechanic / Technician workspace") ? <MechanicExecutionWorkspace organizationName={organizationName ?? "your organization"} role="MECHANIC" /> : session && section === "Technician workspace" ? <MechanicExecutionWorkspace organizationName={organizationName ?? "your organization"} role="TECHNICIAN" /> : session && (section === "Accountant ledger" || section === "P&L analytics") ? <AccountantRoleWorkspace organizationName={organizationName}><AccountantWorkspace /></AccountantRoleWorkspace> : session && section === "Driver portal" ? <DriverWorkspace /> : session && section === "Notifications" ? <NotificationWorkspace /> : session && (section === "Vendors" || section === "Purchase orders") ? <ProcurementWorkspace /> : session && section === "Compliance vault" ? <ComplianceWorkspace /> : session && section === "Billing" ? <BillingWorkspace /> : <section className="replacement-unavailable-workspace"><h2>Workspace unavailable</h2><p>This signed-in role does not have a workspace for the requested route.</p></section>;
