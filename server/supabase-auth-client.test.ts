...
describe("Supabase bearer-token verifier", () => {
  it("uses the public browser-aligned Supabase configuration for getUser", () => {
    expect(source).toContain("const authSupabaseUrl = process.env.VITE_SUPABASE_URL ?? supabaseUrl");
    expect(source).toContain("const authAnonKey = process.env.VITE_SUPABASE_ANON_KEY ?? process.env.SUPABASE_ANON_KEY ?? serviceRoleKey");
