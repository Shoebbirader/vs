...
describe("Razorpay test-mode Starter activation", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.organization.update.mockResolvedValue({ id: "00000000-0000-4000-8000-000000000002", subscriptionTier: "STARTER", maxVehicles: 10, maxUsers: 10 }); mocks.auditEvent.create.mockResolvedValue({ id: "audit" }); mocks.vehicle.count.mockResolvedValue(3); });
  ...
  it("reports an activated Starter tenant as active even when its historical trial end date remains in the future", async () => {
    ...
    expect(status).toMatchObject({ tier: "STARTER", isTrial: false, lifecycle: "ACTIVE", maxVehicles: 10 });
  });
});
