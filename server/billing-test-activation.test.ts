...
const mocks = vi.hoisted(() => ({
  organization: { updateMany: vi.fn(), findFirst: vi.fn() },
...
vi.mock("./razorpay", () => ({
  assertRazorpayTestMode: vi.fn(() => ({
    keyId: "rzp_test_mock",
    keySecret: "secret",
  })),
  createRazorpayTestOrder: vi.fn(),
}));
...
describe("Razorpay test-mode Starter activation", () => {
  beforeEach(() => {
    ...
    mocks.organization.findFirst.mockResolvedValue({
      id: "00000000-0000-4000-8000-000000000002",
  ...
  it("upgrades only the current Superadmin tenant from trial to Starter capacity in test mode", async () => {
    ...
    expect(mocks.organization.updateMany).toHaveBeenCalledWith(
  ...
  it("denies non-Superadmin activation attempts", async () => {
    await expect(
      appRouter
        .createCaller(context("FLEET_MANAGER"))
        .billingTest.activateStarter()
