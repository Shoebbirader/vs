...
const authIssuer = authSupabaseUrl
  ? `${authSupabaseUrl.replace(/\/$/, "")}/auth/v1`
  : null;
...
if (!authSupabaseUrl || !authAnonKey) {
  const message = "[Supabase] SUPABASE_URL and SUPABASE_ANON_KEY are required";
  if (process.env.NODE_ENV === "production") throw new Error(message);
  console.warn(message);
}
...
export async function getSupabaseAuthIdentity(req: Request) {
  ...
  if (!token) {
    console.warn("[Supabase] No bearer token on protected request", {
  ...
  if (tokenIssuer) {
    try {
      ...
      console.warn("[Supabase] Public-key bearer verification failed", {
        path: req?.path ?? req?.url ?? "unknown",
  ...
  if (error || !data.user) {
    console.warn("[Supabase] Bearer token rejected", {
      path: req?.path ?? req?.url ?? "unknown",
...
export async function getFleetOpsUserFromRequest(req: Request) {
  ...
  const user =
    (await fleetDb.user.findUnique({ where: { authUserId: authUser.id } })) ??
    (authUser.email
      ? await fleetDb.user.findFirst({ where: { email: authUser.email } })
      : null);
  if (!user) return null;
  ...
  const normalizedOrg = {
    ...org,
    trialEndsAt:
      org.trialEndsAt instanceof Date
        ? org.trialEndsAt
        : new Date(String(org.trialEndsAt)),
    updatedAt:
      org.updatedAt instanceof Date
        ? org.updatedAt
        : new Date(String(org.updatedAt)),
  };
...
export async function provisionFleetOpsUser(input: {
  ...
  const existing = await fleetDb.user.findUnique({
  ...
  if (input.role && input.role !== "SUPERADMIN") {
    throw new Error(
      "Only SUPERADMIN role is allowed for new organization creation. Other roles must be invited by organization owner."
    );
  }
