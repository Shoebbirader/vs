...
describe("public system diagnostics", () => {
  it("returns a safe release and PostgreSQL dependency status", async () => {
    ...
    expect(result).toMatchObject({ ok: true, release: "vahansync-observability-20260820", database: "ok", clientTimestamp: 123, correlationId: "fleetops-test-correlation-123" });
  ...
  it("exposes release metadata without tenant data", async () => {
    ...
    expect(result).toEqual({ release: "vahansync-observability-20260820", service: "VahanSync API", environment: "development" });
  });
});
