...
try {
  ...
  await check("Inventory Manager can send purchase order", async () => {
    const current = (await tRPC("purchaseOrders.list", invitedToken, null, "GET")).find((row) => row.id === purchaseOrderId);
    return tRPC("purchaseOrders.updateStatus", invitedToken, { id: purchaseOrderId, status: "SENT", expectedUpdatedAt: current.updatedAt });
  });
  const partialReceipt = await check("Inventory Manager can post PO-linked partial receipt", () => tRPC("purchaseOrders.receivePartial", invitedToken, { purchaseOrderId, partId: inventoryPartId, quantity: 2, expectedQuantityOnHand: 1, unitCost: 1400, invoiceNumber: `INV-${runId}-1`, location: "A-01", complete: false }));
  if (partialReceipt.receipt?.quantity !== 2 || partialReceipt.receipt?.invoiceNumber !== `INV-${runId}-1` || partialReceipt.receipt?.location !== "A-01") throw new Error("Partial receipt metadata was not persisted");
  await check("Inventory Manager sees partial-receipt purchase-order state", async () => {
    const current = (await tRPC("purchaseOrders.list", invitedToken, null, "GET")).find((row) => row.id === purchaseOrderId);
    if (current.status !== "PARTIALLY_RECEIVED") throw new Error(`Expected PARTIALLY_RECEIVED, received ${current.status}`);
    return current.status;
  });
  await check("Inventory Manager can post final PO-linked receipt", () => tRPC("purchaseOrders.receivePartial", invitedToken, { purchaseOrderId, partId: inventoryPartId, quantity: 2, expectedQuantityOnHand: 3, unitCost: 1400, invoiceNumber: `INV-${runId}-2`, location: "A-01", complete: true }));
  await check("Inventory balance reflects received quantities", async () => {
    const rows = await tRPC("inventory.list", invitedToken, null, "GET");
    const current = rows.find((row) => row.id === inventoryPartId);
    if (Number(current?.quantityOnHand) !== 5) throw new Error(`Expected quantity 5, received ${current?.quantityOnHand}`);
    return current.quantityOnHand;
  });
  await check("Inventory Manager can close a fully received purchase order", async () => {
    const current = (await tRPC("purchaseOrders.list", invitedToken, null, "GET")).find((row) => row.id === purchaseOrderId);
    const closed = await tRPC("purchaseOrders.updateStatus", invitedToken, { id: purchaseOrderId, status: "CLOSED", expectedUpdatedAt: current.updatedAt });
    if (closed.status !== "CLOSED") throw new Error(`Expected CLOSED, received ${closed.status}`);
  ...
  try {
    ...
    if (orgId) {
      await pool.query(`DELETE FROM purchase_order_receipts WHERE "orgId" = $1`, [orgId]);
