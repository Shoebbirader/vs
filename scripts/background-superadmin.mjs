...
const ownerEmail = `vahansync.e2e.superadmin.${runId}@example.com`;
const password = `VahanSyncSuperadminE2E!${runId}A`;
const invitedEmails = ["FLEET_MANAGER", "INVENTORY_MANAGER", "MECHANIC", "DRIVER", "ACCOUNTANT"].map((role) => `vahansync.e2e.owner.${role.toLowerCase()}.${runId}@example.com`);
const results = [];
