...
The first in-app-routing-only release did not render the disposable invited role workspace within the browser harness timeout. The harness was updated to record the final route and visible state for future failures, while the role guard was hardened to retry a transient first-summary authorization failure after a fresh session refresh instead of falling through into an unverified Home render. The hardened release was pushed to GitHub main and its connected Vercel production deployment started successfully.

The hardened production release was verified with a new disposable Superadmin organization and Fleet Manager invitation. The invited user completed the join form in a fresh browser context, reached the assigned Fleet Manager workspace through in-app routing, and did not trigger a document reload. The harness then removed the temporary Auth identities, organization profile records, invitation, and organization after verification.
