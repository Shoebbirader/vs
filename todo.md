...
- [ ] Repair maintenance automation so installing a component whose existing vehicle odometer already exceeds its threshold immediately creates a VIN-first alert for the Fleet Manager, then verify the Driver odometer update preserves the alert path.
- [ ] Repair the threshold automation work-order payload to include required `id`, `status`, and creation metadata so a valid Driver odometer update completes instead of failing after a due component is evaluated.
- [ ] Repair the Supabase PostgreSQL adapter’s `contains` and `notIn` filter translation so maintenance deduplication and Fleet Manager triage generate valid SQL instead of object-string predicates.
