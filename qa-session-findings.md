...
- The Accountant invitation was redeemed through its live join flow. Its empty Vehicle selector exposed an over-restrictive dependency on the operational `vehicles.list` procedure. A dedicated, role-scoped `financials.vehicles` reference query was implemented, tested, deployed, and verified live: all ten tenant vehicle options are available without exposing vehicle operations. The Accountant then recorded a linked ₹900 toll expense for the primary VIN vehicle; the live ledger and P&L display the expense. The separate maintenance-derived parts/labor financial entries remain pending the real work-order completion handoff.
- The Fleet Manager assigned the active Workflow Lab Driver to the primary vehicle, **VIN VSWFLOW2026000001 / Reg MH 12 VS 0001**, through the live handoff control. The Component workspace initially omitted the inventory parts because it used an Inventory Manager-only list endpoint; a narrow tenant-scoped part reference query was added, tested, released, and then verified live. The Fleet Manager linked the existing Bus Tire part and installed the `Left Tire` lifecycle record with 10,000-km installation, 40,000-km expected life, and 38,000-km alert threshold. At the vehicle’s current 50,000 km, the live VIN-first register displays **Threshold reached**. Notification and work-order handoff verification remains next.
- The initial immediate-evaluation retry exposed two production adapter defects: relation includes were ignored when maintenance evaluation loaded a vehicle, and generated work-order inserts lacked the manual flow’s required fields. A later valid Driver reading also exposed missing PostgreSQL `contains`/`notIn` translation. Each defect was corrected, focused-tested, released through GitHub/Vercel, and re-verified through the live Driver portal. The Driver’s primary vehicle now persists **50,101 km**. The Fleet Manager sees the automated `Left Tire service threshold reached` **CRITICAL / OPEN** order and the recipient-scoped `Maintenance lifecycle alert` with the VIN-first identity. The order has been assigned to the Workflow Lab Mechanic; mechanic execution remains next.
